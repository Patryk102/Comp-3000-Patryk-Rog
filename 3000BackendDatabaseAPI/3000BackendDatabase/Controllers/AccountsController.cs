...
";
        ...
        [Authorize]
        [Route("account/edit/staff")]
        [HttpPut]
        public IActionResult EditStaffAccount([FromBody] dynamic model)
        {
            JObject inputJson;
            try
            {
            ...
            string getSqlString = "SELECT name, surname, email, date_of_birth, password FROM BOOKING.[RestaurantUsers] WHERE user_id = @user_id";
            string[] getParamNames = ["@user_id"];
            string[] getParamValues = [userId];
            JArray getConnectionReturn = new DatabaseConnection(Configuration).GetDatabaseData(getSqlString, getParamNames, getParamValues, true);
            Console.WriteLine("RETURNING A RETURN GET");
            //Console.WriteLine(getConnectionReturn);
            //Console.WriteLine(getConnectionReturn[0]);
            Console.WriteLine(getConnectionReturn[0]["name"]);
            string getPassword = getConnectionReturn[0]["password"].ToString();
            ...
            String connectionString = Configuration.GetConnectionString("Default");
            string sqlString = @"
UPDATE BOOKING.[RestaurantUsers]
SET 
    name = @input_name,
    surname = @input_surname,
    email = @input_email,
    date_of_birth = @input_dateOfBirth,
    password = @input_password
WHERE user_id = @user_id;
;
